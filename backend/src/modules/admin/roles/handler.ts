...
function buildPermissions(raw: Record<string, boolean> = {}): Record<string, boolean> {
...
export async function createRole(
  request: FastifyRequest<{
    Body: { name: string; description?: string; permissions?: Record<string, boolean> };
  }>,
  reply: FastifyReply,
) {
...
export async function updateRole(
  ...
  if (permissions !== undefined) data.permissions = buildPermissions(permissions);
...
export async function deleteRole(
  ...
  const count = await prisma.platformUserRole.count({ where: { roleId } });
  if (count > 0) {
    return reply.status(409).send({
      success: false,
      error: {
