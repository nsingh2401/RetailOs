...
import { lowStockQueue } from '../../jobs/lowStockWorker';
import {
  CreateInvoiceSchema,
  RecordPaymentSchema,
...
export async function createInvoice(
  request: FastifyRequest<{ Params: { storeId: string }; Body: CreateInvoiceInput }>,
  reply: FastifyReply,
) {
  ...
  if (body.localId) {
    const existing = await prisma.invoice.findFirst({ where: { localId: body.localId, storeId } });
...
export async function confirmInvoice(
  request: FastifyRequest<{ Params: { storeId: string; invoiceId: string } }>,
  reply: FastifyReply,
) {
  const storeId   = request.storeId;
  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      if (!invoice)                    throw Object.assign(new Error('Invoice not found'),              { code: 'NOT_FOUND',        statusCode: 404 });
      if (invoice.storeId !== storeId) throw Object.assign(new Error('Invoice not found'),              { code: 'NOT_FOUND',        statusCode: 404 });
      if (invoice.status  !== 'DRAFT') throw Object.assign(new Error('Invoice is not in DRAFT status'), { code: 'INVOICE_NOT_DRAFT', statusCode: 409 });
      ...
      for (const item of invoice.lineItems) {
        ...
        if (available < Number(item.quantity)) {
          throw Object.assign(
            new Error(`Insufficient stock for variant ${item.variantId}. Available: ${available}, Requested: ${item.quantity}`),
            { code: 'INSUFFICIENT_STOCK', statusCode: 422, field: 'lineItems' },
          );
        }
      }
      ...
      const isCredit =
        invoice.paymentMode === 'CREDIT' || Number(invoice.creditAmount) > 0;
  ...
  await lowStockQueue.add('check', { storeId });
...
export async function recordPayment(
  request: FastifyRequest<{ Params: { storeId: string; invoiceId: string }; Body: RecordPaymentInput }>,
  reply: FastifyReply,
) {
  const storeId   = request.storeId;
  const { invoiceId } = request.params;
  ...
  const payment = await prisma.$transaction(async (tx) => {
    ...
    if (!invoice)                    throw Object.assign(new Error('Invoice not found'), { code: 'NOT_FOUND', statusCode: 404 });
    if (invoice.storeId !== storeId) throw Object.assign(new Error('Invoice not found'), { code: 'NOT_FOUND', statusCode: 404 });
    if (invoice.status === 'CANCELLED') throw Object.assign(new Error('Cannot record payment on cancelled invoice'), { code: 'INVOICE_CANCELLED', statusCode: 409 });
...
export async function listInvoices(
  request: FastifyRequest<{ Params: { storeId: string }; Querystring: InvoiceListQuery }>,
  reply: FastifyReply,
) {
  const storeId = request.storeId;
...
export async function cancelInvoice(
  request: FastifyRequest<{ Params: { storeId: string; invoiceId: string }; Body: CancelInvoiceInput }>,
  reply: FastifyReply,
) {
  const storeId   = request.storeId;
  const { invoiceId } = request.params;
  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      if (!['CONFIRMED', 'PARTIAL', 'DRAFT'].includes(invoice.status)) {
        throw Object.assign(
          new Error(`Cannot cancel invoice in status ${invoice.status}`),
