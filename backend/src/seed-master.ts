...
async function seedCategories() {
  ...
  async function seedIndustry(
    industryType: string,
    tree: { root: string; subs: string[] }[],
  ) {
    for (let i = 0; i < tree.length; i++) {
      const parentId = await upsertRoot(
        industryType, tree[i].root, i);
      for (let j = 0; j < tree[i].subs.length; j++) {
        await upsertSub(
          industryType, tree[i].subs[j], parentId, j);
      }
    }
...
async function main() {
  ...
  const [cfgCount, indCount, catCount, brandCount] =
    await Promise.all([
