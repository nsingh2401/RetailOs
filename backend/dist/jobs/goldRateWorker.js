...
exports.fetchGoldRates = fetchGoldRates;
exports.fetchAndCacheGoldRate = fetchGoldRates;
exports.scheduleGoldRateFetch = scheduleGoldRateFetch;
...
async function fetchGoldRates() {
    try {
        ...
        if (!METAL_API_KEY) {
            console.warn('[GoldRate] METAL_PRICE_API_KEY not set — skipping');
            return;
        }
        ...
        const goldUrl = `https://api.metalpriceapi.com/v1/latest`
        ...
        if (!goldData.success ||
            !goldData.rates) {
            console.error('[GoldRate] Gold API error:', goldData);
            return;
        }
        // Fetch silver rates
        const silverUrl = `https://api.metalpriceapi.com/v1/latest`
            + `?api_key=${METAL_API_KEY}`
            + `&base=XAG`
            + `&currencies=${currencyList}`;
        ...
        for (const currency of SUPPORTED_CURRENCIES) {
            const ratePerOz = goldData.rates[currency];
            if (!ratePerOz)
                continue;
            // API returns: 1 XAU = X currency
            // Already in currency per troy oz
            const ratePerGram = ratePerOz / TROY_OZ_TO_GRAM;
            await prisma_1.prisma.fxRate.upsert({
                where: {
            ...
            if (currency === 'INR') {
                await redis_1.redis.setex('jewelry:gold_rate_per_gram_INR', 86400, ratePerGram.toFixed(2));
                // Keep legacy key for compatibility
                await redis_1.redis.setex('jewelry:gold_rate_per_gram', 86400, ratePerGram.toFixed(2));
            }
        }
        // Upsert silver rates
        if (silverData.success &&
            silverData.rates) {
            for (const currency of SUPPORTED_CURRENCIES) {
                const ratePerOz = silverData.rates[currency];
                if (!ratePerOz)
                    continue;
                const ratePerGram = ratePerOz / TROY_OZ_TO_GRAM;
                await prisma_1.prisma.fxRate.upsert({
                ...
                if (currency === 'INR') {
                    await redis_1.redis.setex('jewelry:silver_rate_per_gram_INR', 86400, ratePerGram.toFixed(2));
                    await redis_1.redis.setex('jewelry:silver_rate_per_gram', 86400, ratePerGram.toFixed(2));
                }
            }
        }
        ...
        if (goldInr) {
            const goldInrPerGram = goldInr / TROY_OZ_TO_GRAM;
            await prisma_1.prisma.appConfig.upsert({
                where: {
        ...
        console.log(`[GoldRate] Rates updated for `
            + `${SUPPORTED_CURRENCIES.length}`
            + ` currencies`);
    }
    catch (e) {
        console.error('[GoldRate] fetch error:', e);
    }
}
// Run immediately on startup, then daily at 09:00 AM IST
function scheduleGoldRateFetch() {
    fetchGoldRates();
    node_cron_1.default.schedule('0 9 * * *', () => {
        fetchGoldRates().catch(err => console.error('[GoldRate] cron error:', err));
    }, { timezone: 'Asia/Kolkata' });
}
//# sourceMappingURL=goldRateWorker.js.map