...
function denyIfNotManager(request, reply) {
...
async function getSalesSummary(request, reply) {
    if (denyIfNotManager(request, reply))
        return;
    const storeId = request.storeId;
    const parsed = schema_1.DateRangeQuerySchema.safeParse(request.query);
    if (!parsed.success) {
    ...
    const rawRows = await prisma_1.prisma.$queryRaw(client_1.Prisma.sql `
      SELECT
        invoice_date,
        invoice_count,
        total_revenue_base,
...
async function getTopProducts(request, reply) {
    if (denyIfNotManager(request, reply))
        return;
    const storeId = request.storeId;
    const parsed = schema_1.TopProductsQuerySchema.safeParse(request.query);
    if (!parsed.success) {
        return reply.status(400).send({ success: false, error: { code: 'VALIDATION_ERROR', message: parsed.error.errors[0].message, statusCode: 400 } });
    }
    ...
    const fromDate = new Date(from);
    const toDate = new Date(to);
    const orderCol = sortBy === 'quantity' ? client_1.Prisma.raw('total_quantity') : client_1.Prisma.raw('total_revenue');
    const rawRows = await prisma_1.prisma.$queryRaw(client_1.Prisma.sql `
      SELECT
        ili.variant_id,
        ili.product_name,
...
async function getSlowMovers(request, reply) {
    if (denyIfNotManager(request, reply))
        return;
    const storeId = request.storeId;
    const parsed = schema_1.ReportBaseQuerySchema.safeParse(request.query);
    if (!parsed.success) {
        return reply.status(400).send({ success: false, error: { code: 'VALIDATION_ERROR', message: parsed.error.errors[0].message, statusCode: 400 } });
    }
    const { from, to } = parsed.data;
    const fromDate = new Date(from);
    const toDate = new Date(to);
    const storeRow = await prisma_1.prisma.store.findUnique({
        where: { storeId },
...
async function getTaxSummary(request, reply) {
    if (denyIfNotManager(request, reply))
        return;
    const storeId = request.storeId;
    const parsed = schema_1.ReportBaseQuerySchema.safeParse(request.query);
...
async function getCreditAging(request, reply) {
    if (denyIfNotManager(request, reply))
        return;
    const storeId = request.storeId;
    const parsed = schema_1.ReportBaseQuerySchema.safeParse(request.query);
...
async function getPurchaseSummary(request, reply) {
    if (denyIfNotManager(request, reply))
        return;
    const storeId = request.storeId;
    const parsed = schema_1.ReportBaseQuerySchema.safeParse(request.query);
...
async function getPaymentModes(request, reply) {
    if (denyIfNotManager(request, reply))
        return;
    const storeId = request.storeId;
    const parsed = schema_1.ReportBaseQuerySchema.safeParse(request.query);
...
async function fetchGstr1Data(storeId, fromDate, toDate) {
...
async function fetchGstr3bData(storeId, fromDate, toDate) {
    const rateRows = await prisma_1.prisma.$queryRaw(client_1.Prisma.sql `
    SELECT
      line_tax_rate::int  AS tax_rate,
      SUM(taxable_amount) AS taxable_amount,
      SUM(tax_amount)     AS tax_amount
...
async function getGstr1(request, reply) {
    if (denyIfNotManager(request, reply))
        return;
    const parsed = schema_1.ReportBaseQuerySchema.safeParse(request.query);
    if (!parsed.success) {
...
async function getGstr3b(request, reply) {
    if (denyIfNotManager(request, reply))
        return;
    const parsed = schema_1.ReportBaseQuerySchema.safeParse(request.query);
    if (!parsed.success) {
...
async function buildGstr1Excel(data) {
    ...
    for (const r of data.b2cLarge) {
        ws1.addRow({
            ...
            blended_tax_rate: Number(r.blended_tax_rate ?? 0),
        });
    }
    ...
    for (const r of data.b2cSmall) {
        ws2.addRow({ tax_rate: Number(r.tax_rate ?? 0),
            taxable_amount: Number(r.taxable_amount ?? 0),
...
async function buildGstr3bExcel(data) {
    const wb = new exceljs_1.default.Workbook();
...
async function buildSalesExcel(storeId, fromDate, toDate) {
    const rawRows = await prisma_1.prisma.$queryRaw(client_1.Prisma.sql `
...
async function exportReport(request, reply) {
    if (denyIfNotManager(request, reply))
        return;
    const parsed = schema_1.ExportQuerySchema.safeParse(request.query);
    if (!parsed.success) {
        return reply.status(400).send({ success: false,
            error: { code: 'VALIDATION_ERROR',
                message: parsed.error.errors[0].message, statusCode: 400 } });
    ...
    if (format === 'json') {
        let data;
        if (type === 'gstr1')
            data = await fetchGstr1Data(storeId, fromDate, toDate);
        else if (type === 'gstr3b')
            data = await fetchGstr3bData(storeId, fromDate, toDate);
        else {
            const rawRows = await prisma_1.prisma.$queryRaw(client_1.Prisma.sql `
        SELECT invoice_date, invoice_count, total_revenue_base,
               total_tax, total_discount, unique_customers
    ...
    let buffer;
    if (type === 'gstr1')
        buffer = await buildGstr1Excel(await fetchGstr1Data(storeId, fromDate, toDate));
    else if (type === 'gstr3b')
        buffer = await buildGstr3bExcel(await fetchGstr3bData(storeId, fromDate, toDate));
    else
        buffer = await buildSalesExcel(storeId, fromDate, toDate);
    reply.header('Content-Disposition', `attachment; filename="${label}.xlsx"`);
    reply.header('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    return reply.send(buffer);
}
// â”€â”€ Shared: fetch Tally line-item data â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
async function fetchTallyData(storeId, fromDate, toDate) {
...
function buildTallyXml(rows, tallyVersion) {
...
async function getTallyExport(request, reply) {
    if (denyIfNotManager(request, reply))
        return;
    const parsed = schema_1.TallyExportQuerySchema.safeParse(request.query);
    if (!parsed.success) {
        return reply.status(400).send({ success: false,
            error: { code: 'VALIDATION_ERROR',
                message: parsed.error.errors[0].message, statusCode: 400 } });
    ...
    try {
        ...
        console.error('[TallyExport] fetchTallyData error:', err.stack ?? err);
        return reply.status(500).send({
            success: false,
            error: { code: 'TALLY_FETCH_ERROR', message: err.message, statusCode: 500 },
        });
    }
    if (format === 'csv') {
