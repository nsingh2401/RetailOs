...
exports.identifyProduct = identifyProduct;
...
async function identifyProduct(request, reply) {
    // ── Read uploaded image ───────────────────────────────────
    ...
    // ── HSN enrichment ────────────────────────────────────────
    const rawHsn = raw.hsn_code_suggestion ?? '';
    let hsnCode = '';
    let hsnDescription = '';
    let defaultGstRate = null;
    if (rawHsn.length >= 4) {
        try {
            const hsn = await prisma_1.prisma.hSNCode.findFirst({
