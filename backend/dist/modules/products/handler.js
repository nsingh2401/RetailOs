...
exports.lookupByBarcode = lookupByBarcode;
...
function canManageProduct(request) {
...
function buildBarcodeResult(variant, product, inventory) {
    return {
...
async function buildIndexDoc(productId) {
    const p = await prisma_1.prisma.product.findUnique({
        where: { productId },
...
async function listProducts(request, reply) {
    ...
    const parsed = schema_1.ProductListQuerySchema.safeParse(request.query);
    if (!parsed.success) {
...
async function createProduct(request, reply) {
    ...
    const parsed = schema_1.CreateProductSchema.safeParse(request.body);
    if (!parsed.success) {
        return reply.status(400).send({ success: false, error: { code: 'VALIDATION_ERROR', message: parsed.error.message, statusCode: 400 } });
    }
    ...
    try {
        const result = await prisma_1.prisma.$transaction(async (tx) => {
            ...
            let resolvedHsnCode = body.hsnCode ?? undefined;
            if (resolvedHsnCode) {
        ...
        buildIndexDoc(result.product.productId).then((doc) => {
            if (doc)
                void (0, typesense_1.indexProduct)(doc);
        }).catch(() => { });
...
async function searchProducts(request, reply) {
    const { storeId } = request.params;
    const parsed = schema_1.ProductSearchQuerySchema.safeParse(request.query);
    if (!parsed.success) {
    ...
    try {
        ...
        // unavailable
        const rawProducts = await prisma_1.prisma.$queryRaw `
      SELECT
...
async function lookupByBarcode(request, reply) {
...
async function updateProduct(request, reply) {
    ...
    const parsed = schema_1.UpdateProductSchema.safeParse(request.body);
    if (!parsed.success) {
        return reply.status(400).send({ success: false, error: { code: 'VALIDATION_ERROR', message: parsed.error.message, statusCode: 400 } });
    }
    const body = parsed.data;
    // Ensure product belongs to this store
    const existing = await prisma_1.prisma.product.findFirst({ where: { productId, storeId } });
    if (!existing) {
        return reply.status(404).send({ success: false, error: { code: 'PRODUCT_NOT_FOUND', message: 'Product not found', statusCode: 404 } });
    }
    try {
...
async function deleteProduct(request, reply) {
    ...
    void (0, typesense_1.deleteProductIndex)(productId);
    return reply.send({ success: true, data: { productId, deleted: true } });
}
// ── Variant handlers ───────────────────────────────────────────
// POST /:storeId/products/:productId/variants
async function createVariant(request, reply) {
    const { storeId, productId } = request.params;
    if (!canManageProduct(request)) {
        return reply.status(403).send({ success: false, error: { code: 'FORBIDDEN', message: 'MANAGER or OPERATOR with product permission required', statusCode: 403 } });
    }
    const parsed = schema_1.CreateVariantSchema.safeParse(request.body);
    if (!parsed.success) {
        return reply.status(400).send({ success: false, error: { code: 'VALIDATION_ERROR', message: parsed.error.message, statusCode: 400 } });
    }
    const body = parsed.data;
    const product = await prisma_1.prisma.product.findFirst({ where: { productId, storeId, isActive: true } });
    if (!product) {
        return reply.status(404).send({ success: false, error: { code: 'PRODUCT_NOT_FOUND', message: 'Product not found', statusCode: 404 } });
    }
    try {
...
async function updateVariant(request, reply) {
    const { storeId, productId, variantId } = request.params;
    if (!canManageProduct(request)) {
        return reply.status(403).send({ success: false, error: { code: 'FORBIDDEN', message: 'MANAGER or OPERATOR with product permission required', statusCode: 403 } });
    }
    const parsed = schema_1.UpdateVariantSchema.safeParse(request.body);
    if (!parsed.success) {
        return reply.status(400).send({ success: false, error: { code: 'VALIDATION_ERROR', message: parsed.error.message, statusCode: 400 } });
    }
    const body = parsed.data;
    const existing = await prisma_1.prisma.productVariant.findFirst({
    ...
    if (body.barcode !== undefined && existing.barcode && body.barcode !== existing.barcode) {
        await (0, redis_1.cacheDelete)(`barcode:${storeId}:${existing.barcode}`);
    }
    // Invalidate new barcode cache too (stale data may exist)
    if (body.barcode) {
        await (0, redis_1.cacheDelete)(`barcode:${storeId}:${body.barcode}`);
    }
    try {
    ...
    for (const cat of allCategories) {
        ...
        if (cat.parentId) {
            ...
            if (parent)
                ...
                roots.push(node); // orphaned — surface as root
        }
        else {
            roots.push(node);
        }
    }
    await (0, redis_1.cacheSet)(cacheKey, roots, 600);
...
async function createCategory(request, reply) {
    ...
    try {
        ...
        await (0, redis_1.cacheDelete)(`categories:${storeId}`);
...
async function updateCategory(request, reply) {
    ...
    const existing = await prisma_1.prisma.category.findFirst({ where: { categoryId, storeId } });
    if (!existing) {
...
async function deleteCategory(request, reply) {
    ...
    await prisma_1.prisma.category.delete({ where: { categoryId } });
    await (0, redis_1.cacheDelete)(`categories:${storeId}`);
...
async function createBrand(request, reply) {
    ...
    const parsed = schema_1.CreateBrandSchema.safeParse(request.body);
    if (!parsed.success) {
        return reply.status(400).send({ success: false, error: { code: 'VALIDATION_ERROR', message: parsed.error.message, statusCode: 400 } });
    }
    const body = parsed.data;
...
async function saveProductImage(request, reply) {
    const { storeId, productId } = request.params;
    const parsed = schema_1.SaveProductImageSchema.safeParse(request.body);
    if (!parsed.success) {
        return reply.status(400).send({ success: false, error: { code: 'VALIDATION_ERROR', message: parsed.error.message, statusCode: 400 } });
    }
    const body = parsed.data;
