...
async function listUsers(request, reply) {
    ...
    const [users, total] = await Promise.all([
        prisma_1.prisma.platformUser.findMany({
            where,
            include: {
                userRoles: {
                    include: {
        ...
        prisma_1.prisma.platformUser.count({ where }),
    ]);
    return reply.send({
        success: true,
        data: {
...
async function createUser(request, reply) {
    ...
    const existing = await prisma_1.prisma.platformUser.findUnique({
...
async function deleteUser(request, reply) {
    ...
    try {
        await prisma_1.prisma.platformUser.update({ where: { id: userId }, data: { isActive: false } });
